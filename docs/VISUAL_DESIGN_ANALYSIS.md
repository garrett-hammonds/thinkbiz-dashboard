...
color, elevation/shadow, and consistency against the `CLAUDE.md` style guide.
...
`CLAUDE.md` is the brand source of truth. Its rules are explicit: a teal /
...
`CLAUDE.md` defines **Secondary = Navy Blue** and specifies the primary button as
...
1. **Standardize page-title typography.** Page `<h1>`s vary across the app —
   ...
   (apply, profile) vs `text-4xl font-black` (support). `CLAUDE.md` defines a
...
4. **Spacing rhythm pass.** `CLAUDE.md` prescribes section padding (`py-12 lg:py-16`)
